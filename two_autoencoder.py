    ...
    for tetrode_number in [9]:
        ...
        try:
            for i in range(NUM_EPOCHS):
                ...
                if (i+1)%100==0:
                    codes = training['code'](dataset['X_train'])
                    print(codes.shape)
                    ...
                    plt.scatter(codes[:, 0], codes[:, 1], c=dataset['y_train_labels'],alpha=0.8,lw=0)

                    plt.savefig('../logs/auto/test_{}.png'.format(i+1), bbox_inches='tight')
                    plt.close()

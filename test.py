...
# Dimshuffle from (batch_size, seq_length, n_outputs) to 
# (batch_size, n_outputs, seq_length) so we convolve over the
# time axis.
l_dimshuffle1 = DimshuffleLayer(l_recurrent, pattern=(0, 2, 1))

# convolve over the time axis
l_conv1d = lasagne.layers.Conv1DLayer(
    l_dimshuffle1, num_filters=5, filter_size=2, stride=1, nonlinearity=None,
    border_mode='same')

"""
Below is a quick demo of using Conv1D to convolve over the time axis.  This 
could be used as the basis for hierarchical subsampling (see Chapter 9 of 
Alex Graves, 'Supervised Sequence Labelling with Recurrent Neural Networks').
"""

# back to (batch_size, seq_length, n_outputs)
l_dimshuffle2 = DimshuffleLayer(l_conv1d, pattern=(0, 2, 1))

l_recurrent2 = lasagne.layers.RecurrentLayer(l_dimshuffle2, N_HIDDEN, nonlinearity=None)
...
l_reshape = lasagne.layers.ReshapeLayer(l_recurrent2, (N_BATCH*LENGTH, N_HIDDEN))

...
def main(tetrode_number=TETRODE_NUMBER):
    ...
    if(os.path.isfile('recurrent_argmax')):
        print("Loading old model")
        f=open('recurrent_argmax','r')
        all_param_values = pickle.load(f)
        f.close()
        lasagne.layers.set_all_param_values(network, all_param_values)
    else:
        print('recurrent_argmax doesnt exist')
        return 
    ...
    for z in range(10):
        try:
            ...
            plt.savefig('../position/argmax/costs_{}_{}.png'.format(i,z), bbox_inches='tight')
            plt.close()
        except IndexError:
            break
    ...
    for i,(actual,prediction) in enumerate(zip(actuals,predictions)):
        ...
        plt.savefig('../position/argmax/Position_{}.png'.format(i), bbox_inches='tight')
        plt.close()
...
def makeVideo(prediction, actual, points_from, number):
    ...
    animation.write_gif("../position/argmax/position_{}.gif".format(number), fps=fps)
